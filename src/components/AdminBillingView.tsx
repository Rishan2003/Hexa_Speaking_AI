...
export const AdminBillingView: React.FC = () => {
  ...
  const setCreditCost = (mode: keyof typeof DEFAULT_CREDIT_COSTS, value: number) => {
    setSettings((current) => ({
      ...current,
      creditCosts: {
        ...DEFAULT_CREDIT_COSTS,
        ...(current.creditCosts || {}),
        [mode]: Math.max(0, Math.floor(Number(value) || 0)),
      },
    }));
  };
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--hexa-red)]">Administration</p><h1 className="mt-1 text-3xl font-black text-slate-950">Paid test controls</h1><p className="mt-1 text-sm text-slate-500">Manage signup credits, per-test credit costs, packages, user balances, unlimited access and payment history.</p></div>
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <label className="text-xs font-bold text-slate-600">Free credits on signup<input type="number" min="0" value={settings.signupFreeTests ?? 0} onChange={(e)=>setSettings({...settings, signupFreeTests:Number(e.target.value)})} className="mt-1 block w-full rounded-xl border border-slate-200 px-3 py-2.5"/></label>
            ...
            <div className="sm:col-span-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-xs font-black text-slate-800">Credit cost by practice type</p><p className="mt-1 text-[11px] text-slate-500">Set 0 to make a practice type free. Unlimited accounts are never charged.</p></div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Server enforced</span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
                {([['part1','Part 1'],['part2','Part 2'],['part3','Part 3'],['full','Full Mock']] as const).map(([mode,label]) => (
                  <label key={mode} className="text-xs font-bold text-slate-600">{label}<input type="number" min="0" step="1" value={settings.creditCosts?.[mode] ?? DEFAULT_CREDIT_COSTS[mode]} onChange={(e)=>setCreditCost(mode, Number(e.target.value))} className="mt-1 block w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5"/></label>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="rounded-3xl border border-[rgba(47,51,127,.14)] bg-[var(--hexa-soft-blue)] p-5">
          <h2 className="text-sm font-black text-[var(--hexa-navy)]">Account creation behavior</h2><p className="mt-2 text-xs leading-relaxed text-slate-600">Changing the signup allowance affects new entitlement records only. Existing balances are not rewritten. Test credit costs apply to future reservations; sessions already created keep their reserved cost.</p>
        </div>
      </section>
